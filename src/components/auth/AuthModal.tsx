...
export const AuthModal: React.FC = () => {
  ...
  // Official Google Identity Services (GIS) Client OAuth Integration with Polling Retry
  useEffect(() => {
  ...
  }, [isAuthModalOpen, authStep, rememberMe]);
      ...
      <div className="flex flex-col gap-5 p-1">
        ...
        {authStep === 'otp' && (
          <div className="flex flex-col gap-5 animate-in fade-in duration-200">
            ...
            {generatedOtp && (
              <div className="p-3 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-xs text-slate-200">
                  <Sparkles className="w-4 h-4 text-cyan-400 shrink-0 animate-pulse" />
                  <span>Kode Simulasi: <strong className="text-cyan-300 font-mono text-sm tracking-widest">{generatedOtp}</strong></span>
                </div>
                <button
                  type="button"
                  onClick={handleAutoFillOtp}
                  className="px-2.5 py-1 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 text-[11px] font-bold transition-colors"
                >
                  Tempel
                </button>
              </div>
            ...
            <div className="flex justify-center items-center gap-2 sm:gap-3 my-2">
              {otpValues.map((digit, idx) => (
                <input
                  ...
                  autoFocus={idx === 0}
                  className="w-11 h-13 sm:w-12 sm:h-14 text-center text-lg sm:text-xl font-bold font-mono rounded-2xl bg-[#0B0C10] border border-white/20 text-white focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 transition-all"
                />
              ))}
            </div>
        ...
        {authStep === 'form' && (
          <>
            ...
            <div className="flex flex-col items-center text-center gap-1.5">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 flex items-center justify-center shadow-lg shadow-cyan-500/25 text-white mb-1">
            ...
            <div className="flex flex-col items-center justify-center pt-1 min-h-[44px]">
              ...
              <div ref={googleBtnRef} className={`w-full flex justify-center ${!gisLoaded ? 'hidden' : ''}`} />
              
              {/* Fallback & Instant Google Login Button */}
              {!gisLoaded && (
                <button
                  type="button"
                  onClick={handleTriggerGoogleOAuth}
                  className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-2xl bg-white hover:bg-slate-100 text-slate-900 font-semibold text-xs transition-all shadow-md hover:shadow-lg active:scale-[0.99] border border-slate-200"
                >
                  ...
                  <span>{authMode === 'login' ? 'Masuk dengan Akun Google' : 'Daftar dengan Akun Google'}</span>
                </button>
              )}
            </div>
